...
function Carousel({ title, livros, estoquePorLivro, fotoPorLivro, autorPorLivro, generoPorLivro, onVerLivro }) {
	...
	const hasNavigation = livros.length > ITEMS_PER_VIEW;

	function scroll(dir) {
		if (!hasNavigation) return;
		const step = dir === "left" ? -ITEMS_PER_VIEW : ITEMS_PER_VIEW;
		setStartIndex((prev) => (prev + step + livros.length) % livros.length);
	}

	const visibleLivros = useMemo(() => {
		if (!hasNavigation) return livros;
		return Array.from(
			{ length: ITEMS_PER_VIEW },
			(_, idx) => livros[(startIndex + idx) % livros.length]
		);
	}, [livros, startIndex, hasNavigation]);
	...
	useEffect(() => {
		if (!hasNavigation || pauseAutoplay) return undefined;
		const interval = setInterval(() => {
			setStartIndex((prev) => (prev + ITEMS_PER_VIEW) % livros.length);
		}, 3500);
		return () => clearInterval(interval);
	}, [hasNavigation, pauseAutoplay, livros.length]);
		...
		<section className="panel carousel-panel">
			<div className="carousel-header-row">
				...
				<div className="carousel-actions">
					<button type="button" className="carousel-btn" onClick={() => scroll("left")} aria-label="Anterior" disabled={!hasNavigation}>
						<FontAwesomeIcon icon={faChevronLeft} />
					</button>
					<button type="button" className="carousel-btn" onClick={() => scroll("right")} aria-label="Próximo" disabled={!hasNavigation}>
			...
			<div
				...
				style={{ "--carousel-columns": String(Math.min(ITEMS_PER_VIEW, visibleLivros.length)) }}
				onMouseEnter={() => setPauseAutoplay(true)}
				onMouseLeave={() => setPauseAutoplay(false)}
			>
				{visibleLivros.map((livro) => (
					<LivroCard
						key={livro.idLivro}
						livro={livro}
